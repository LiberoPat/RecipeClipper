...
final class WhatINeedViewModel {
    ...
    init(weekStart: Int64, groceries: GroceryRepository, pantry: PantryRepository, preferences: AppPreferences) {
        ...
        loading = Task { [weak self] in
            ...
            self.subscription = pantry.observeItems()
                .receive(on: DispatchQueue.main)
                .sink { [weak self] items in self?.uiState.needs = PantryMatch.weekNeeds(sources, pantry: items) }
        }
    }
